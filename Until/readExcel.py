...
class readExcel():
    ...
    def load_excel(self, excelFile=None):
        ...
        excelFile = openpyxl.load_workbook(path + excelPath)
        return excelFile


    def get_sheet_data(self, sheetName=None):
        """
        获取指定sheet的内容
        :param index:
        :return:
        ...
        data = self.load_excel().get_sheet_by_name(sheetName)
        return data

    def get_cell_value(self, row, cols, sheetname):
        """
        获取某一个单元格内容
        :param row:
        :param cols:
        :return:
        """
        data = self.get_sheet_data(sheetname).cell(row=row, column=cols).value
        return data
    ...
    def get_columns_value(self, key=None, sheetName=None):
        """
        获取某一列
        :param key:
        :return:
        ...
        if key == None:
            key = 'A'
        columns_list_data = self.get_sheet_data(sheetName)[key]
        for i in columns_list_data:
            columns_list.append(i.value)
        return columns_list

    def get_rows_number(self, case_id, sheetName=None):
        """获取行号"""
        num = 1
        cols_data = self.get_columns_value(sheetName=sheetName)
