...
def _build_name(name, index, xx):
    return '%s_%d_%s' % (name, index, xx)
...
def parametrized(*parameters_seq):
    ...
    def magic_module_set_test_case(cls):
        ...
        # lxh_name = cls.__name__.split('.')[0]
        # print(cls.__name__.split('.'))
        for index, parameters in enumerate(parameters_seq):
            name = _build_name(cls.__name__, index, parameters[0][1])
